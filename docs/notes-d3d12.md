...
在 D3D12 中，“签名”的含义完全一样。它就是 GPU 渲染管线的“函数参数列表声明”。

通过根签名，你用 C++ 显式地告诉 GPU：“接下来的着色器（Shader）是一个函数，这个函数需要用到哪些资源（纹理、常量、采样器等）”。

在现代 GPU 硬件中，存储这些“参数”的地方，叫做 “根寄存器” (Root Registers)。这是显卡芯片上最珍贵、读写速度最疯狂、离计算核心最近的超高速缓存区域（通常只有 64 个 DWORD 的大小，也就是 256 字节左右）。
